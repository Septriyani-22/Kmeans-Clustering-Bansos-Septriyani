...
<div style="background:#fff; border-radius:12px; box-shadow:0 2px 12px rgba(0,0,0,0.07); padding:32px 40px; max-width:900px; margin:0 auto;">
    ...
    <form action="{{ route('kepala_desa.penduduk.update', $penduduk->id) }}" method="POST">
        @csrf
        @method('PUT')

        @include('kepala_desa.penduduk._form', ['penduduk' => $penduduk])
        ...
        <a href="{{ route('kepala_desa.penduduk.index') }}" style="background:#888fa6; color:#fff; padding:10px 18px; border-radius:6px; text-decoration:none; margin-left:10px;">Batal</a>
    </form>
</div>
@endsection
